...
- 현재 Phase: **Phase 8.6 — 신호 생성 로직 구조 재설계 (Sprint 1·2·3·4 완료, Sprint 5 진단 Sprint 진행 중)**
- 현재 Sprint: **Phase 8.6 Sprint 5 — 🔄 진행 중 (2026-05-14 계획 수립)** — 진단·측정 Sprint (T1 코드 즉답 + T2 DB/백테스트 + T3 라이브 WS trace 1주 병행, 임계 변경 0건)
...
- **Sprint 5** 🔄 (2026-05-14 계획 수립) — 진단·측정 Sprint: 2026-05-13~14 모니터링에서 발견된 14개 결함 중 미해결 7건(#6 KIS WS 35% 누락 / #8 R1 발동 / #9 G3 부등호 / #10 breakout 72.2% 편중 / #11 stage 직렬 AND / #13 fallback 폭증 / #14 secondary 4h 100% 교체)을 T1(코드 즉답) + T2(walk-forward 재활용 + M-F2 DB 쿼리) + T3(라이브 WS trace, Paper 1주 병행)로 본질 측정. 임계/dry_run/LIVE 변경 0건, Hotfix A(PR #237) + B(PR #238) 머지 완료, T1 결과로 Hotfix C(#9) 조건부 분리. Phase 8.7 entry gate 3개(E1 WS≤5% / E2 fallback≤20% / G-Bt1+G-Bt2) 평가 — `docs/phase/phase8.6/sprint5/sprint5.md` (5 Task, 3~5일 + T3 병행 1주)
