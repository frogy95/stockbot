...
### 목표
장전 전 종목 일괄 수집(공공데이터포털) + 장중 실시간 수집(한투 API) 2단계 수집 체계 구축. 체결강도 직접 계산 모듈 구현. 종목 스크리닝 엔진 및 스코어링.

### 작업 목록
#### Sprint 1: 데이터 수집 시스템
- 수집 스케줄러 (APScheduler, 장전/장중/장후)
- **장전 일괄 수집**: 공공데이터포털 → 전 종목(~2,880개) 종가/거래량/시총 DB 저장 (6회 API 호출)
- 한투 API 시세 데이터 수집 (1차 스크리닝 통과 종목만) + market_data 테이블 저장
- **체결강도 계산 모듈**: 웹소켓 체결 데이터 기반 직접 계산 (현재가 API에서 미제공)
- DART 재무 기초 데이터 연동 (재무정보만, 공시 모니터링 제외)
- DART corp_code 매핑 테이블 구축 (corpCode.xml → DB)
- 네이버 뉴스 센티멘트 배치 수집 모듈 (보조 팩터, 일 1~2회)
- 수집 데이터 Redis 캐싱 (실시간 시세 TTL 5초)
- **ETF 별도 수집**: 공공데이터포털 미제공 → 한투 REST 개별 조회

#### Sprint 2: 종목 스크리닝 엔진
- 1차 스크리닝 (장전, DB 기반): 거래량, 시가총액, 등락률, 변동성 필터
- 2차 스크리닝 (장중, 실시간): 호가 잔량, 체결강도, 분봉 패턴
- 종목 스코어링 알고리즘 (팩터 기반, ETF 전용 팩터 분리)
- 스크리닝 결과 DB 저장 + Redis 캐싱
- stocks 마스터 테이블 관리 (KOSPI/KOSDAQ + ETF)
- 재무 데이터 미제공 종목 팩터 폴백 처리
...
- ETF: 공공데이터포털 미제공 → 한투 REST 개별 조회 + 재무 팩터 제외
