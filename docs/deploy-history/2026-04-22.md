...
프론트엔드 `resetRiskCounters()` 함수가 `{ ok: boolean, message: string }` 응답을 기대하였으나 백엔드가 `risk_status` dict를 반환하는 계약 불일치로 리셋 버튼이 항상 실패 메시지를 표시하던 문제 수정. 백엔드 리셋 동작 자체는 정상이었음.

변경 파일: `frontend/lib/api.ts`, `frontend/components/risk/reset-button.tsx`

### 자동 검증 결과

- ✅ TypeScript `npx tsc --noEmit`: PR #165 CI 통과
