...
### 프로덕션 배포 - v2.8.0 (2026-04-29) — Phase 8.6 Sprint 2 관찰 결과 포함
...
#### 관찰 항목 (Sprint 3 착수 게이트) — 2026-04-30 판정 결과

- ⚠️ **G1: ATR 캘리브레이션 잡 (Redis 4종 키 적재)** — 측정 불가 (로그 보존 범위 밖, `railway run` 권한 제한)
- ❌ **G2: 병렬 OR tier 신호 발생** — 신호 0건 (2026-04-28/29/30 연속). G2 자동 롤백(R1) 발동 → Redis override 설정 (KST 16:10, 2026-04-30)
- ⚠️ **G3: 시뮬-실측 절대차 ≤0.15** — 측정 불가 (G2 신호 0건으로 분모=0)
- ✅ **L5 사전 시뮬**: fail율 추정 35~45% < 60% → ATR_FLOOR=0.025 시작값 유지

**Sprint 3 착수 판정: NO-GO** — 근본 원인(`prev_close_volume_confirm` 게이트 키 형식 불일치) hotfix 배포 후 1거래일 재관찰 필요
