...
PR: https://github.com/frogy95/stockbot/pull/182 (develop → main, 머지: 2026-04-29T04:19:13Z)
...
- ✅ Vercel 프론트엔드 접속: HTTP 200 (https://stockbot.choiji.kr)
- ✅ Alembic 마이그레이션: `upgrade a430a1c931b2 -> b8f1c2a30201, Phase 8.6 Sprint 1 — fallback 컬럼 추가 (TradeSignal·Order)` 성공
- ✅ 서버 기동: KIS 클라이언트 초기화 완료, APScheduler 잡 등록, WS 연결 20개 정상
- ✅ 로그인 페이지 렌더: `StockBot` 타이틀 + 비밀번호 입력창 정상 표시
- ⬜ Playwright `/diagnostics` 페이지 — `FallbackSignalRateCard` + `AutoRollbackMultiTrigger` 렌더: 로그인 비밀번호 필요, 수동 확인 필요
...
- Paper/dry_run 모드 유지 — LIVE 전환은 Sprint 2 DoR 4종 통과 후
