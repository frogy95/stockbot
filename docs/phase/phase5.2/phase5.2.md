...
> **Status**: Sprint 1 ✅ 완료 (2026-04-08, PR #106)
...
## Sprint 1 상세 — WS 재연결 안정화 + 구독 제한 ✅ 완료 (PR #106, 2026-04-08)
...
| 6 | _reconnect() 구독 복원 중 예외 발생 시 수신 루프 미시작 (연결 누수) | ⚠️ 중간 | Sprint 1에서 미해결 — Sprint 2 개선 권장: 구독 복원 실패 시 현재 연결을 명시적으로 닫거나 수신 루프를 구독 복원 전에 시작 |
...
| 항목 | 기준 | 상태 |
|------|------|------|
| 모의 환경 WS 재연결 반복 해소 | 장중(09:00~15:30) 연속 1시간 안정 연결 유지 | ⬜ 수동 검증 필요 (프로덕션 배포 후) |
| 2차 스크리닝 30초 주기 정상 실행 | 10회 연속 실행 확인 (5분간) | ⬜ 수동 검증 필요 (프로덕션 배포 후) |
| 환경별 구독 제한 동작 | paper=25, live=35 자동 적용 | ✅ 완료 (pytest test_ws_manager_env_max_subscriptions 통과) |
| WS 재연결 실패 시 텔레그램 알림 | 7회 실패 시 알림 발송 확인 | ✅ 완료 (pytest test_reconnect_calls_failure_callback 통과) |
| REALTIME_CACHE_TTL 10초 적용 | Redis TTL 확인 | ✅ 완료 (scheduler.py REALTIME_CACHE_TTL=10 확인) |
| 재연결 후 체결강도 웜업 | 5초 무시 구간 동작 확인 | ✅ 완료 (pytest test_trade_strength_warmup 통과) |
| 테스트 전체 통과 | pytest 회귀 테스트 Green | ✅ 완료 (751 passed, 2026-04-08) |
