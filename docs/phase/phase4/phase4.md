...
## Sprint 2 상세 -- 신호/스크리닝/설정 + 웹 매매 승인
...
|---|------|--------|------|-------------|------|
| 1 | CORS 프로덕션 URL 누락 | ❌ 높음 | 윤에이피 | Sprint 1 | 환경변수 `ALLOWED_ORIGINS` 도입 |
| 2 | 모드 전환 보호 부재 | ❌ 높음 | 최리스크 | Sprint 2 | 이중 확인 + 장중 차단 + 포지션 체크 |
| 3 | 색상 혼동 (서양식 적용 위험) | ⚠️ 중간 | 한유엑 | Sprint 1 | `lib/colors.ts` 색상 상수 강제 |
| 4 | Sprint 2 범위 과다 | ⚠️ 중간 | 정프로 | Sprint 2 | 웹 승인을 Sprint 2 초반에 배치 |
| 5 | 텔레그램/웹 동시 승인 경쟁 | ⚠️ 중간 | 윤에이피 | Sprint 2 | 기존 ApprovalManager 일회용 토큰 활용 |
| 6 | 신호 페이지 폴링 3초 부하 | ⚠️ 낮음 | 윤에이피 | Sprint 2 | 승인 대기 존재 시에만 3초, 없으면 5초 |
| 7 | shadcn/ui 초기 설정 시간 | ⚠️ 낮음 | 정프로 | Sprint 1 | Sprint 1 Task 1에 충분한 시간 배분 |
| 8 | 성과 분석 범위 혼동 | ⚠️ 낮음 | 정프로 | Sprint 2 | Phase 4: 일별 테이블만, Phase 5: 차트/통계 |
...
| 항목 | 기준 | 상태 |
|------|------|------|
| 8개 페이지 접근 가능 | 메인/포지션/주문/신호/스크리닝/이력/분석/설정 | ⬜ |
| 데이터 표시 | 모든 페이지에서 백엔드 데이터 정상 렌더링 | ⬜ |
| 웹 매매 승인/거부 | 승인 대기 신호에 대해 웹에서 승인/거부 동작 | ⬜ |
| 실전/모의 시각 구분 | 배너 + 배지 + 인디케이터 3중 표시 | ⬜ |
| 모드 전환 웹 전용 | 모의<->실전, 반자동<->완전자동 전환이 웹에서만 가능 | ⬜ |
| 모드 전환 보호 | 이중 확인 + 장중 차단 + 포지션 체크 | ⬜ |
| 인증 | JWT 로그인, 비인증 접근 차단 확인 | ⬜ |
| API 응답 시간 | 95th percentile < 500ms | ⬜ |
| CORS | 로컬 + 프로덕션 URL 모두 정상 | ⬜ |
| 색상 관례 | 빨강=수익/상승, 파랑=손실/하락 | ⬜ |
| 리스크 설정 장중 잠금 | UI에서 장중 리스크 설정 비활성화 | ⬜ |
| 폴링 동작 | 5초 기본, 신호 승인 대기 시 3초, 탭 비활성화 시 중단 | ⬜ |
