...
| 범위 확장 | 전략 수정만 | **+ 5분봉 거래량 수집 파이프라인 선행 구축** | Phase 7 데이터 의존성 해소를 위해 이 Phase에서 축적 시작 |
...
Phase 7에서 "5분 단위 거래량 가속도 지표"를 구현하려면 최소 20거래일의 5분봉 거래량 데이터가 필요하다. Phase 7 착수 시점에 데이터가 없으면 추가 20거래일(약 1개월)을 기다려야 한다. 이를 방지하기 위해 **이 Phase에서 수집 파이프라인을 미리 구축하고 축적을 시작**한다.
...
**TTL**: 30일 (약 22거래일분). Phase 7 착수 시점에 충분한 데이터 확보 후 DB 이관 검토
...
- Phase 7 착수 시점에 `vol5m:*` 키로 축적된 데이터를 읽어 거래량 가속도 지표 계산
- 사용은 Phase 7부터. 이 Phase에서는 **축적만 수행**, 전략 로직에서 참조하지 않음
- 축적 미완 상태에서 Phase 7 착수 지시가 오면 AI가 경고 발생
