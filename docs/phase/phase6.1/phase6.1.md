...
[확정 설계]
  ...
  (변경)
    ...
    # 절대 거래량 하한: 전일의 30% 미만이면 유동성 부족으로 탈락
    if snapshot.volume < snapshot.prev_volume * MIN_VOLUME_FLOOR:
        return None
    
    # 시간가중 보정: "현 시점 기준 마감 예상 거래량 비율"
    volume_ratio = snapshot.volume / (snapshot.prev_volume * progress)
    if volume_ratio < 2.0: return None
```
...
| # | 항목 | 원래 설계 | 확정값 | 근거 |
|---|------|----------|--------|------|
| 1 | volume_ratio 임계값 | 2.0 (하드코딩) | **2.0 유지** | 전원 합의. 시간보정으로 해결하며 임계값 자체는 "마감 기준 2배 예상" 의미 보존 |
...
| 5 | MIN_VOLUME_FLOOR | 없음 | **0.3** (prev_volume * 0.3) | 최리스크+김단타: 전일 30% 미만 거래량은 유동성 부족, 시간보정과 무관 탈락 |
...
| 파일 | 작업 내용 |
|------|----------|
| `backend/modules/trading/strategies/momentum_breakout.py` | (1) `calc_market_progress()` 함수 추가 (2) MIN_MARKET_PROGRESS=0.15, MIN_VOLUME_FLOOR=0.3 상수 (3) 거래량 조건 블록 교체: 절대하한 체크 + 시간가중 보정 + 기존 2.0 임계값 (4) volume_score도 adjusted_ratio 사용 |
| `backend/modules/trading/strategies/__init__.py` | 변경 불필요 (import 변경 없음) |
| `backend/tests/test_momentum_breakout.py` | (1) 기존 테스트 volume_ratio 케이스 수정 (보정 반영) (2) 신규: 시간대별 보정 정확도 테스트 (09:30/11:00/14:00) (3) 신규: min_progress 하한 테스트 (4) 신규: 절대 거래량 하한 테스트 (5) 신규: 장 시간 외 호출 시 안전 동작 테스트 |
...
MIN_VOLUME_FLOOR = 0.3       # 전일 거래량의 30% 미만이면 유동성 부족 탈락
...
| # | 항목 | 심각도 | 대응 |
|---|------|--------|------|
| 1 | 선형 보정은 장 중반(11~13시)에 ~10~20% 보수적 | 낮음 | 보수적 방향이므로 리스크 관리에 유리. Phase 7에서 비선형 검토 가능 |
...
| 4 | 장기 개선: 시간대별 거래량 프로파일 | Phase 7+ | 20거래일 데이터 축적 후 U자형 보정 검토 (박퀀트 권고) |
| 5 | 장기 개선: 거래량 가속도 지표 | Phase 7+ | WS 틱데이터 기반 5분 단위 가속도 (김단타 권고) |
...
| 안전장치 구현 | MIN_MARKET_PROGRESS + MIN_VOLUME_FLOOR | ⬜ |
| 단위 테스트 | 기존 케이스 수정 + 시간대별 보정 5건 이상 | ⬜ |
| 프로덕션 배포 | Railway 배포 + 장중 1건 이상 신호 생성 확인 | ⬜ |
| 모니터링 | 3거래일 로그 확인 (adjusted_ratio 분포) | ⬜ |
