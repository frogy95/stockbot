...
| 6 | trade_strength_min 60 → 65 상향 시점 | 정보 | 김단타+최리스크 | LIVE 1주 관찰 후 결정 |
...
| 항목 | 기준 | 상태 |
|------|------|------|
| P0 결함 3건 수정 | 가격 갱신 + 포지션 생성 + 청산 실행 동작 확인 | ⬜ |
| P1 이중 주문 방지 | cancel 실패 시 return 동작 확인 | ⬜ |
| P1 파라미터 조정 | trade_strength_min=60, max_candidates=20 반영 | ⬜ |
