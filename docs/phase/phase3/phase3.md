...
| 손절 (레버리지) | 미설정 | **-1%** | 일반의 절반, 변동성 반영 | 최리스크 |
| 익절 (일반) | +3% | **+3%** (유지) | Phase 1 확정값 | 최리스크+김단타 |
| 익절 (레버리지) | 미설정 | **+2%** | 변동성 반영 축소 | 최리스크 |
| 트레일링 스탑 | -1% 즉시 | **-1%, +1.5% 이상 시 활성화** | 조기 청산 방지 | 최리스크 |
...
| 3 | 텔레그램 장애 시 승인 폴백 | 정프로 | Phase 3에서는 자동 만료만, 웹 승인은 Phase 4 | Sprint 3 |
...
| 8 | Railway 재배포 시 웹훅 URL 변경 | 윤에이피 | 앱 시작 시 setWebhook API 자동 호출 | Sprint 3 |
...
| 텔레그램에서 승인/거부 시 주문 실행/취소 확인 | 텔레그램 봇 E2E 테스트 | -- |
| 승인 타임아웃(30초/15초) 시 자동 만료 + 알림 | Redis TTL + 키스페이스 노티피케이션 | -- |
| 주문 실행 지연 < 1초 (한투 API 호출까지) | 성능 측정 | -- |
| 알림 지연 < 3초 (신호 발생 -> 텔레그램 수신) | 성능 측정 | -- |
| 모의거래 환경에서 전체 매매 사이클 테스트 완료 | 통합 테스트 | -- |
| 조회 명령어(/status, /today, /mode, /help) 동작 | 텔레그램 봇 테스트 | -- |
