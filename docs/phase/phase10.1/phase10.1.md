# Phase 10.1: 신호 생성 로직 구조 재설계 — 실행 계획

> **Status**: 계획 수립 완료 (2026-04-28, 사용자 승인 대기)
> **ROADMAP 참조**: ROADMAP.md `Phase 10.1` 절 — 본 문서 승인 시 "누적 백로그 통합" → "신호 생성 로직 구조 재설계 (분기 D 트리거 선제 착수)"로 재정의되며, 기존 백로그(피라미딩 / 2차 하이브리드)는 Phase 10.2로 분리한다.
...
- **단, Phase 10.1 → Phase 10.2 (구 백로그) 분리 정책 신규**: 본 Phase가 "누적 백로그 통합"이 아닌 **"분기 D 구조 재설계 선제 착수"**로 재정의되므로, 피라미딩·2차 스크리닝 하이브리드는 Phase 10.2로 별도 분리한다. (사용자 승인 시 ROADMAP·index.json 동시 반영)

---

## 2. 목표 / 성공 기준 (정량)

### 2.1 시스템 행동 변경 목표 (Phase 10.1 완료 시점)
...
- 피라미딩(당일 고가 갱신 진입) → Phase 10.2로 분리
- 2차 스크리닝 절대 점수 ↔ 백분위 하이브리드 → Phase 10.2로 분리
- VI 재개 tier (`vi_resume`) → Phase 10.2 또는 별도 후속 (단타 3순위, 변동성 리스크가 커서 1·2순위 정착 후 도입)
- 테마 모멘텀 가중치 (`theme_momentum`) → Phase 10.2로 분리 (섹터 메타데이터 인프라 별도 필요)
...
- 트리거 발동 시 **Phase 10.1 신규 변경분만 비활성화**, Phase 8.5 폴백은 별도 결정
...
> **의도**: PO가 "Phase 10.1 점프 대신 1주 내 재배포"로 권고한 안을 본 Phase의 첫 Sprint로 흡수. 분기 D 손실을 빠르게 막고, Sprint 2~4의 전제조건(DoR G1~G3)을 동시에 깐다.
...
| Phase 10.1 점프 vs 즉시 재배포 — Sprint 1 흡수 결정 | PO §3 (Sprint 2.6 안) — 사용자 구조 재설계 선택 후 흡수 |
...
> **참고**: Phase 10.1 → Phase 10.2 분리(기존 백로그 이관)는 본 Phase 전제로 **이미 채택**되어 §1.4 / §5 / §6 / §10 전반에 반영됨. 사용자가 명시적으로 거부할 경우에만 본 문서 절반의 재작성이 필요하므로 별도 결정 항목이 아닌 **착수와 동반 적용**으로 처리.

---

## 10. 완료 기준 (Phase 10.1 전체 DoD)
...
1. **승인 → Phase 10.1 Sprint 1 sprint-planner 호출** (DoR 가드레일 G1~G3 + PO Sprint 2.6 흡수 + Sprint 1)
2. **특정 파라미터 수정 후 승인** — 예: Sprint 3 volume_surge position 30% → 25% 보수 강화, walk-forward 60일 → 90일 등
3. **거부 / 재검토** — Sprint 1만 별도 핫픽스로 분리하고 Phase 10.1 본격 착수는 더 보수적 일정으로
...
- §9.3 사용자 최종 결정 필요 항목 (Phase 10.2 분리, KIS 분봉 백필 분리, dry_run 토글 권한)
