...
- ⬜ Playwright `/diagnostics` 페이지 — 로그인 비밀번호 필요, 수동 확인 필요

#### 수동 검증 필요 항목

- ⬜ Playwright `/diagnostics` 페이지 접속 — `FallbackSignalRateCard` + `AutoRollbackMultiTrigger` 렌더 확인 (로그인 후 수동)
- ⬜ Paper 모드 1거래일 회귀: `signals.fallback=true` 1건 이상 DB 기록 + M-F2 API 응답 정상 (장 마감 후 확인)
- ⬜ G2/G3 가드레일 실 동작 확인: 16:10 자동 롤백 잡 + 회로차단기 counter 정상 적재 (장 후 확인)
