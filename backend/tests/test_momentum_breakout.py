...
@pytest.mark.asyncio
async def test_no_breakout_returns_none():
    """전일 고가 미돌파 -> None 반환."""
    from modules.trading.strategies.momentum_breakout import MomentumBreakoutStrategy
...
@pytest.mark.asyncio
async def test_low_volume_returns_none():
    """거래량 조건 미달 -> None 반환."""
    from modules.trading.strategies.momentum_breakout import MomentumBreakoutStrategy
...
@pytest.mark.asyncio
async def test_low_trade_strength_returns_none():
    """체결강도 조건 미달 -> None 반환."""
    from modules.trading.strategies.momentum_breakout import MomentumBreakoutStrategy
...
@pytest.mark.asyncio
async def test_gap_switches_to_daily_high():
    """갭 3%+ 시 당일 고가 기준으로 전환."""
    from modules.trading.strategies.momentum_breakout import MomentumBreakoutStrategy
...
@pytest.mark.asyncio
async def test_atr_filter_excludes_high_volatility():
...
@pytest.mark.asyncio
async def test_confidence_weighted_average():
    ...
    volume_score = min(40000000 / 10000000 / 5.0, 1.0)
...
@pytest.mark.asyncio
async def test_stop_loss_take_profit_normal():
...
@pytest.mark.asyncio
async def test_stop_loss_take_profit_leverage():
    """레버리지 종목 손절/익절: -1.5%/+3%."""
    from modules.trading.strategies.momentum_breakout import MomentumBreakoutStrategy
...
@pytest.mark.asyncio
async def test_reason_dict_structure():
